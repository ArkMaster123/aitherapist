...
export async function GET() {
  ...
  // Class name is KokoroTTS, Modal converts it to kokorotts (lowercase, no dashes)
  const wsUrl = `wss://${modalUsername}--${modalAppName}-kokorotts-web.modal.run/ws`;
  const httpUrl = `https://${modalUsername}--${modalAppName}-kokorotts-web.modal.run/tts`;
